...
import { GraphyGraphComponent,GraphEdgeArray, GraphEdge } from './graphy-graph.component';
...
describe('GraphyGraphComponent', () => {
  ...
  it('nodes are correct', () => {
    ...
    //   }) ? true : false;
    // });
    expect(component.nodes.length).toEqual(node_data.length);
  });
  ...
  it('edges are correct', () => {
    ...
    //     return ((local_edge.source == component_edge.source) &&
    //             (local_edge.target == component_edge.target));
    //   }) ? true : false;
    // });
    expect(component.edges.length).toEqual(node_data.length-1);
  });
});
